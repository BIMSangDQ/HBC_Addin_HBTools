...
namespace BeyCons.Core.RevitUtils.DataUtils.Filters
{
    public class BasicFilters
    {
        ...
        public BasicFilters(List<Element> elements)
        {
                ...
                if (element.Category != null)
                {
                    switch (element.Category.Id.IntegerValue)
                    {
                        ...
                        case (int)BuiltInCategory.OST_IOSModelGroups:
                            List<Element> elementsGroup = (element as Autodesk.Revit.DB.Group).GetMemberIds().Select(x => RevitData.Instance.Document.GetElement(x)).ToList();
