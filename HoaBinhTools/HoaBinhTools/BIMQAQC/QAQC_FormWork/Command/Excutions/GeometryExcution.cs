...
namespace HoaBinhTools.BIMQAQC.QAQC_FormWork.Command.Excutions
{
	public class GeometryExcution
	{
		...
		public FaceType GetAllFaceFormworkElement(Element element, Solid elementSolid, IList<Element> intersectElements, ref int numberOfFaceError)
		{
			...
			IEnumerable<Face> originalFaces = GeometryUtils.GetFacesFromSolid(elementSolid).Where(f => f.Area > FormworkViewModels.EpsilonArea);
			...
			if (originalPlanarAndCylindricalFaces.Count() > 0)
			{
				...
				foreach (Face planarAndCylindricalFace in originalPlanarAndCylindricalFaces)
				{
					if (planarAndCylindricalFace is PlanarFace planarFace)
					{
						try
						{
							Solid solidFormwork = GeometryUtils.CreateSolidFromPlanarFace(element.Document, elementSolid, planarFace, FormworkViewModels.Thickness);
							...
							else
							{
								...
								if (solidFormwork.Volume > FormworkViewModels.EpsilonVolume)
								{
									PlanarFace planarFaceOrigin = FormworkUtils.GetPlanarFace(planarFace, GeometryUtils.GetFacesFromSolid(solidFormwork).OfType<PlanarFace>(), FormworkViewModels.EpsilonArea);
									if (null != planarFaceOrigin)
									{
										if (planarFaceOrigin.Area > FormworkViewModels.EpsilonArea)
										{
					...
					else
					{
						try
						{
							Solid solidFormwork = GeometryUtils.CreateSolidFromCylindricalFace(planarAndCylindricalFace as CylindricalFace, FormworkViewModels.Thickness);
							IList<Element> elementIntersectSolidsFormwork = IntersectElement.GetElemenstIntersectWithSolid(solidFormwork, intersectElements);
							List<Solid> solidIntersectSolidsFormwork = elementIntersectSolidsFormwork.Select(e => GeometryUtils.GetSolidsFromInstanceElement(e, FormworkViewModels.Options, true).ToUnionSolids()).SelectMany(x => x).ToList().ToUnionSolids();
							...
							if (solidFormwork.Volume > FormworkViewModels.EpsilonVolume)
							{
								CylindricalFace cylindricalFaceOrigin = FormworkUtils.GetCylindricalFace(GeometryUtils.GetFacesFromSolid(solidFormwork).OfType<CylindricalFace>(), elementSolid, FormworkViewModels.EpsilonArea);
								if (null != cylindricalFaceOrigin)
								{
									if (cylindricalFaceOrigin.Area > FormworkViewModels.EpsilonArea)
									{
										if (cylindricalFaceOrigin.Area < planarAndCylindricalFace.Area + FormworkViewModels.EpsilonArea)
										{
		...
		public FaceType GetAllFaceFormworkElement(Element element, Solid elementSolid)
		{
			...
			if (null != solidOpening)
			{
				try
				{
					...
					return new FaceType() { OriginFaces = GeometryUtils.GetFacesFromSolid(elementSolid).Where(a => a.Area > FormworkViewModels.EpsilonArea).ToList(), RegainFaces = new List<Face>() };
				}
				catch
				{
					return new FaceType() { OriginFaces = GeometryUtils.GetFacesFromSolid(elementSolid).Where(a => a.Area > FormworkViewModels.EpsilonArea).ToList(), RegainFaces = new List<Face>() };
		...
		private bool IntersectFormworkSolid(ref Solid formworkSolid, IList<Element> intersectElements)
		{
			...
			{
				Solid solid = GeometryUtils.GetSolidsFromInstanceElement(intersectElement, FormworkViewModels.Options, true).ToUnionSolid();
				if (null != solid)
				{
					try
					{
			...
			if (null != unionSolids && unionSolids.Count == 1)
			{
				try
				{
					...
					foreach (Face otherFaceSolidUnionAll in otherFacesSolidUnionAll)
					{
						...
						{
							if (otherFaceSolidUnionAll.Area > FormworkViewModels.EpsilonArea)
							{
		...
		public List<Solid> UnionSolidsIntersectElement(Element element, Solid elementSolid, IList<Element> intersectElements)
		{
			...
			foreach (Element intersectElement in intersectElements)
			{
				solid = GeometryUtils.GetSolidsFromInstanceElement(intersectElement, FormworkViewModels.Options, true).ToUnionSolid();
				if (null != solid)
				{
					solids.Add(solid);
				}
		...
		{
			if (element.Category.Name == FormworkViewModels.Categories[0].Name)
			{
				return GetOpeningsSolidFromColumn(element);
			}
			else if (element.Category.Name == FormworkViewModels.Categories[1].Name)
			{
				return GetOpeningsSolidFromWall(element);
			}
			else if (element.Category.Name == FormworkViewModels.Categories[2].Name)
			{
				...
				foreach (Opening opening in rectangularArcWallOpenings)
				{
					...
					if (null != solidOpening)
					{
						...
						if (cylindricalFaces.Count > 0)
						{
							...
							{
								solidOpeningWalls.Add(GeometryUtils.CreateSolidFromCylindricalFace(cylindricalFace, FormworkViewModels.Thickness));
							}
						}
					}
					else
