			...
			foreach (List<Element> Framing in EleFramings)
			{
				...
				foreach (var eleInter in elesIntersect)
				{
					try
					{
						...
						if (eleInter.Category.Id.IntegerValue == (int)BuiltInCategory.OST_StructuralFraming)
						{
							...
							if (HoaBinhTools.Properties.Settings.Default.IsSecondaryBeam == true)
							{
								// dầm diện tích bé hơn
								if (Host.Section - TP > 0.001)
								{
							...
							{
								// dầm diện tích bé hơn
								if (TP - Host.Section <= 0.0001)
								{
									Adj.Type = TypeAdjSupport.DP;
								}
								else if (TP < Host.Section)
								{
									Adj.Type = TypeAdjSupport.DP;
								}
								// dầm diện tích lớn hơn
					...
					catch (Exception ex)
					{
						...
						FraInfoViewModels.ListErro.Add(new ErrorModels() { ErrorID = eleInter.Id, Category = eleInter.Category.Name, InfoErro = ex.ToString() });
					}
				}
				...
				foreach (Element Beam in Host.ListHost)
				{
					...
					foreach (var curBemaPan in Curs)
					{
						try
						{
							// ActiveData.Document.DrawingCurve(curBemaPan);
						...
						{
							FraInfoViewModels.ListErro.Add(new ErrorModels() { ErrorID = Beam.Id, Category = string.Format("Host: {0}", Beam.Name), InfoErro = ex.ToString() });
						}
					}
		...
		public ObservableCollection<BeamSystemsGroup> UpdataBeamSystemsGroup(string IDGroup, ObservableCollection<BeamSystemsGroup> SystemFramings)
		{
			...
			foreach (var SysFami in SystemFramings)
			{
				...
				else
				{
					...
					ProgressBarView UptateGoi = new ProgressBarView();
					...
					foreach (var Support in elesIntersect)
					{
						...
						if (!UptateGoi.Create(elesIntersect.Count, " Update Goi ")) break;
					...
					ProgressBarView UptateNhip = new ProgressBarView();

					UptateNhip.Show();

					foreach (Element Beam in Host.ListHost)
					{
						if (!UptateGoi.Create(Host.ListHost.Count, " Update Goi ")) break;
