...
namespace HoaBinhTools.FramingInformation.Models.CreateRebar
{
	public class AddT1
	{
		public void DrawAddRebar(List<MovableRestBeam> ListMovable, Document doc)
		{
			...
			int slthepchu = ListMovable[0].FraInfoViewModels.SL_T1; //Số lượng thép chủ
																	//Đếm số nhịp
			int SoluongNhip = 0;
			...
			try
			{
				for (int i = 0; i < ListMovable.Count; i++)
				{
					...
					if (movable.Type.ToString() == "ND")
					{
						...
						origin = origin - vectorZ * abvtop; // Đưa về đúng cao độ đặt thép rồi
						...
						foreach (DbAddT1 span in movable.FraInfoViewModels.ListAddT1)
						{
