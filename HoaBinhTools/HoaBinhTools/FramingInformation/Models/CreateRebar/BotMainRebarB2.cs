...
{
	public class BotMainRebarB2
	{
		public void DrawBotMainBar(List<MovableRestBeam> ListMovable, Document doc)
		{
			#region Variable
			RebarBarType rebarBarType = new FilteredElementCollector(doc)
				.OfClass(typeof(RebarBarType))
				.Cast<RebarBarType>()
				.First(x => x.Name == ListMovable[0].FraInfoViewModels.BotMainBarDia_B2);
			...
			double Lneo = double.Parse(ListMovable[0].FraInfoViewModels.Bot_Anchorage) * dkthepchu;
			...
			try
			{
				for (int i = 0; i < ListMovable.Count; i++) // Lặp qua từng nhịp
				{
					...
					if (movable.Type.ToString() == "ND")
					{
						...
						origin = origin + vectorY * abv - vectorZ * (h - abv - MathUtils.MmToFoot(50));
						...
						if (Nhip == 0)
						{
							...
							else// Có gối phía trước
							{
								...
								{
									Khoanglui = MathUtils.MmToFoot(250);
								}
								...
								if (ChieuDaiGoi > Lneo) // Neo thẳng
								{
									...
									//CreateRebar(curves,abv,movable.FraInfoViewModels.SL_B2,vectorY,rebarBarType,doc,elem,b);
								}
								else // Có bẻ giò gà
								{
									// Tính lại khoảng neo vào để đảm bảo giò gà
									double doangioga = Lneo - (ChieuDaiGoi - Khoanglui);
									if (doangioga < 12 * dkthepchu)
									{
										doangioga = 12 * dkthepchu;
						...
						{
							if (Math.Abs(elBot - Ele_old) <= MathUtils.MmToFoot(50)) // Trường hợp nhấn thép không cắt thép hoặc chạy thawnrr đc
							{
							...
							{
								if (elBot - Ele_old > 0) // Nhịp hiện tại neo thẳng, nhịp củ giò gà lên hoặc neo thẳng luôn
								{
									...
									else // Có bẻ giò gà
									{
										...
										sectionPnts[0] = sectionPnts[1];
										sectionPnts[1] = sectionPnts[0] + vectorX * (ChieuDaiGoi - Khoanglui);
										Line ln = Line.CreateBound(sectionPnts[0], sectionPnts[1]);
										curves.Add(ln); // Đoạn neo gio ga

										sectionPnts[0] = sectionPnts[1];
										sectionPnts[1] = sectionPnts[0] + vectorZ * doangioga;
										ln = Line.CreateBound(sectionPnts[0], sectionPnts[1]);
										curves.Add(ln); // Đoạn neo
									...
									#region Đoạn neo thanh mới
									sectionPnts[0] = origin - vectorX * Lneo;
									sectionPnts[1] = origin + vectorX * nhipdam;
									Line lr = Line.CreateBound(sectionPnts[0], sectionPnts[1]);
									curves.Add(lr); // Đoạn neo vào gối
									#endregion
								}
								else // Giật cấp xuống thanh cũ tính neo, thanh mới neo thẳng
								{
									...
									#endregion
									CreateRebar(curves, abv, movable.FraInfoViewModels.SL_B2, vectorY, rebarBarType, doc, elem, b); // VẼ  thanh cũ
