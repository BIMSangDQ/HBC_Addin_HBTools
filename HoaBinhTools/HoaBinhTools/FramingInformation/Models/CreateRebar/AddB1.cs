...
namespace HoaBinhTools.FramingInformation.Models.CreateRebar
{
	public class AddB1
	{
		public void DrawAddRebar(List<MovableRestBeam> ListMovable, Document doc)
		{
			...
			int slthepchu = ListMovable[0].FraInfoViewModels.SL_B1; //Số lượng thép chủ
																	//Đếm số nhịp
			int SoluongNhip = 0;
			...
			try
			{
				for (int i = 0; i < ListMovable.Count; i++)
				{
					...
					if (movable.Type.ToString() == "ND")
					{
						...
						origin = origin - vectorZ * (h - abvtop); // Đưa về đúng cao độ đặt thép rồi
						...
						foreach (DbAddB1 span in movable.FraInfoViewModels.ListAddB1)
						{
