...
{
	public class AddhorizontalLinkBot
	{
		public void DrawHorizontalLine(List<MovableRestBeam> ListMovable, Document doc)
		{
			...
			int slT2 = ListMovable[0].FraInfoViewModels.SL_B2;

			int sl1 = 0;
			int sl2 = 0;
			int sl3 = 0;

			double tl = double.Parse(Save.Default.TL_bot);
				...
				for (int i = 0; i < ListMovable.Count; i++)
				{
					...
					if (movable.Type.ToString() == "ND")
					{
						...
						foreach (Line V in LL)
						{
							...
							sectionPnts[0] = V.GetEndPoint(0) + vectorY * abvtop + vectorX * khoanglui - vectorZ * (h - abvtop - MathUtils.MmToFoot(45));
							sectionPnts[1] = V.GetEndPoint(0) + vectorY * (b - abvtop) +
											 vectorX * khoanglui - vectorZ * (h - abvtop - MathUtils.MmToFoot(45));
