...
namespace HoaBinhTools.FramingInformation.Models.CreateRebar
{
	public class AddB2
	{
		public void DrawAddRebar(List<MovableRestBeam> ListMovable, Document doc)
		{
			...
			int slthepchu = ListMovable[0].FraInfoViewModels.SL_B2; //Số lượng thép chủ
																	//Đếm số nhịp
			int SoluongNhip = 0;
			...
			double tl = double.Parse(Save.Default.TL_bot);  // Thay bằng tỉ lệ setting
			int Nhip = 0;
			...
			try
			{
				for (int i = 0; i < ListMovable.Count; i++)
				{
					...
					if (movable.Type.ToString() == "ND")
					{
						...
						double elBot = MathUtils.MmToFoot((double)movable.TopElevation);
						...
						origin = origin - vectorZ * (h - abvtop - MathUtils.MmToFoot(50)); // Đưa về đúng cao độ đặt thép rồi
						...
						foreach (DbAddB2 span in movable.FraInfoViewModels.ListAddB2)
						{
						...
						if (sl1 != 0) // ĐẦu nhịp có thép thì tạo curve
						{
							...
							Lneo = double.Parse(ListMovable[0].FraInfoViewModels.Bot_Anchorage) * rebarBarType.BarDiameter;
							...
							else //Không phải nhịp đầu tiên vẽ curve ra
							{
								if (curves.Count != 0) //Nếu đã tồn tại curve
								{
									...
									{
										//Giật cấp lên thanh cũ bẻ giò gà
										if (elBot - Ele_old > 0)
										{
											...
											sectionPnts[1] = origin + vectorX * nhipdam * tl;
											Line lr = Line.CreateBound(sectionPnts[0], sectionPnts[1]);
											curves.Add(lr); // Đoạn neo  thẳng vào gối
											#endregion

										}
										// Giật cấp xuống thanh cũ neo thẳng, thanh mới bẻ giò gà
										else
										{
											#region  Đoạn neo thanh cũ
											double ChieuDaiGoi = MathUtils.MmToFoot((ListMovable[i + 1].Length));
						...
						{
							Lneo = double.Parse(ListMovable[0].FraInfoViewModels.Bot_Anchorage) * cu.BarDiameter;
