...
{
	public class Ver_Stirrup
	{
		public void DrawStirrup(List<MovableRestBeam> ListMovable, Document doc)
		{
			#region Variable
			...
			RebarHookType HookStart =
				(from abcd in new FilteredElementCollector(doc).OfClass(typeof(RebarHookType)).Cast<RebarHookType>()
					...
					if (movable.Type.ToString() == "ND")
					{
						...
						foreach (SStirrup span in movable.FraInfoViewModels.ListSStirrup)
						{
								...
								&& span.Type == "Dai")
							{
								...
								int vitri2 = int.Parse(span.Vitri2.ToString());

								double kcthanh = (b - 2 * abv) / (slthepchu - 1);
								List<XYZ> sectionPnts = new List<XYZ>
								{
								origin + vectorY*(abv+kcthanh*(vitri-1)-MathUtils.MmToFoot(25)) - vectorZ * abvtop,
								origin + vectorY*(abv+kcthanh*(vitri2-1)+MathUtils.MmToFoot(25))- vectorZ * abvtop,
								origin + vectorY*(abv+kcthanh*(vitri2-1)+MathUtils.MmToFoot(25)) - vectorZ * (h-abvtop),
								origin + vectorY*(abv+kcthanh*(vitri-1)-MathUtils.MmToFoot(25))- vectorZ * (h-abvtop),
								};
